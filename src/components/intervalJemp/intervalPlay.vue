<template>
    <div class="mb-6 mt-6 transport-grid">
        <v-btn color="success" height="52" class="depth-btn" @click="$emit('playAgain')">
            <v-icon v-if="started" size="24">mdi-replay</v-icon>
            <span v-else>{{ $t('common.start') }}</span>
        </v-btn>
        <v-btn class="button depth-btn" color="error" height="52" :disabled="!started" @click="$emit('playRandomInterval')">
